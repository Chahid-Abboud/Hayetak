...
export default function MealHistoryCalendar({
  ...
  const weeks = useMemo(() => {
    ...
    for (let d = 1; d <= days; d++) cells.push({ ymd: ymd(year, monIndex + 1, d), day: d });
  ...
  return (
    <div className="rounded-2xl border border-[#1C2C64]/20 bg-white p-4 dark:bg-[#0B1020] dark:border-white/20">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm font-semibold">
          {new Date(year, monIndex, 1).toLocaleString(undefined, { month: "long", year: "numeric" })}
        </div>
        <button
          className="rounded-lg border px-2 py-1 text-sm dark:border-white/20"
          onClick={onNextMonth}
      ...
      <div role="grid" className="mt-2 grid grid-cols-7 gap-1">
        {weeks.flat().map((c, i) => {
          if (!c.ymd) return <div key={i} className="h-9 rounded-lg" />;
          ...
          return (
            <button
              ...
              className={[
                "h-9 rounded-lg text-sm transition border",
                isSelected ? "bg-[#1C2C64] text-white border-[#1C2C64]" : "bg-transparent border-transparent hover:border-[#1C2C64]/30",
              ].join(" ")}
              aria-label={`${c.ymd}${hasEntries ? " (has meals)" : ""}`}
            >
              <span className="inline-flex items-center justify-center gap-1">
                {c.day}
                {hasEntries ? <span className="text-[10px]">•</span> : null}
              </span>
            </button>
          );
        })}
