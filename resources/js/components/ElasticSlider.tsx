  ...
  return (
    <div className="w-full">
      <div className="mb-2 flex items-end justify-between">
        ...
        <motion.span
          className="rounded-md px-2 py-0.5 text-xs font-semibold bg-primary/10 text-primary"
          aria-live="polite"
        >
        ...
        */}
        <div
          className="
            ...
            bg-[color:var(--sidebar)]/15
            dark:border-transparent
            dark:bg-[linear-gradient(to_right,var(--sidebar),var(--primary))]
          "
        />
        ...
        <div className="pointer-events-none absolute inset-x-0 -top-1 h-4">
          <div className="relative h-full">
            {ticks.map((t) => {
              ...
              return (
                <div key={t.v} className="absolute" style={{ left: `calc(${pct}% - 1px)` }}>
                  <div className={`w-0.5 ${t.large ? "h-3 bg-foreground/50" : "h-2 bg-foreground/30"}`} />
                  {t.large && (
        ...
        <input
          id={id}
          type="range"
          className="absolute inset-0 h-2 w-full appearance-none bg-transparent cursor-pointer"
          min={min}
          max={max}
          step={step}
          value={local}
