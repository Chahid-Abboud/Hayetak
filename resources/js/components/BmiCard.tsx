...
function category(bmi: number): {
    ...
    if (bmi < 18.5)
        return {
            key: 'under',
            label: 'Underweight',
            hint: 'Aim to gain safely.',
        };
    if (bmi < 25)
        return {
            key: 'normal',
            label: 'Normal range',
            hint: 'Great—keep it up!',
        };
    if (bmi < 30)
        return {
            key: 'over',
            label: 'Overweight',
            hint: 'Small changes help.',
        };
    ...
    switch (key) {
        ...
        case 'over':
            return {
                ...
                body: 'Tracking meals is the fastest way to create a consistent calorie deficit. We’ll keep it simple and steady.',
                href: '/meal-tracker',
        ...
        case 'under':
            return {
                ...
                body: 'Increase calories with nutrient-dense meals and gentle strength work to gain safely.',
                href: '/meals',
        ...
        default: // normal
            return {
                ...
                body: 'You’re in the healthy range. A simple workout routine and good hydration will help you maintain.',
                href: '/workout',
...
export default function BmiCard({ isGuest, profile, loading }: Props) {
    ...
    if (!isFinite(height_m) || height_m <= 0)
        return (
            <div className="text-muted-foreground">Height looks invalid.</div>
        );

    const bmi = round1(profile.weight_kg / (height_m * height_m));
    if (!isFinite(bmi) || bmi <= 0)
        return (
            <div className="text-muted-foreground">Weight looks invalid.</div>
        );
    ...
    return (
        <div>
            ...
            <div
                className={`mt-3 rounded-lg border p-3 ${cta.toneClass}`}
                role="status"
                aria-live="polite"
            >
