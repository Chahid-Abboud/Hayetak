...
export default function AdminOverviewIndex() {
  ...
  const stats: AdminOverviewStat[] = [
    { label: 'Users', value: 1240, tone: 'accent', helper: 'Total registered' },
    { label: 'Pending Verifications', value: 23, helper: 'Awaiting review' },
  ...
  const priorityQueue: AdminOverviewPriorityItem[] = [
    {
      ...
      description: 'Planner suggested peanuts despite user allergy profile',
      severity: 'danger',
      meta: 'Plan ID: 7891',
    },
    {
      ...
      description: 'Professional verification shows license expired 2025-11-03',
      severity: 'warning',
      meta: 'Role: Trainer',
    },
    {
      ...
      description: 'Coach recommended heavy lifting for user with back injury',
      severity: 'warning',
      meta: 'Chat #5542',
    },
  ];
  ...
  const operations: AdminOverviewOperations[] = [
    { label: 'Notifications', value: 12, icon: Bell, href: '/admin/notifications' },
    { label: 'Logs', value: 34, icon: Settings2, href: '/admin/logs' },
    { label: 'Support Cases', value: 8, icon: Users, href: '/admin/support-cases' },
  ];
          ...
          {priorityQueue.length > 0 ? (
            <AdminScrollArea maxHeightClassName="max-h-[300px]">
              <AdminDataTable>
                <ProductTableHead>
                  <tr>
                    <ProductTableHeaderCell className="w-4">Severity</ProductTableHeaderCell>
                    <ProductTableHeaderCell>Title</ProductTableHeaderCell>
                    <ProductTableHeaderCell className="w-32">Action</ProductTableHeaderCell>
                  </tr>
                </ProductTableHead>
                <ProductTableBody>
                  {priorityQueue.map((item) => (
                    <ProductTableRow key={item.id}>
                      <ProductTableCell>
                        <StatusChip value={item.severity} label={item.severity} />
                      </ProductTableCell>
            ...
            <AdminEmpty title="No priority items" description="All queues are clear." />
          )}
        </AdminSection>
        ...
        <AdminSection title="Attention Summary">
          <div className="grid gap-4 sm:grid-cols-3">
            {attentionCards.map((card, index) => (
              <AdminOverviewCard key={index} title={card.title} description={card.description}>
        ...
        <AdminSection title="Quick Operations">
          <div className="grid gap-4 sm:grid-cols-3">
            {operations.map((op, index) => (
              <AdminOverviewCard key={index} title={op.label} description={`${op.value} items`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <op.icon className="h-5 w-5" />
                    <div className="text-2xl font-semibold tracking-tight text-foreground">{op.value}</div>
                  </div>
                  <Link href={op.href} className="text-sm text-muted-foreground hover:text-foreground">
