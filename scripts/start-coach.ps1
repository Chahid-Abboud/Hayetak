...
if (-not (Test-Endpoint -Url 'http://127.0.0.1:11434/api/tags')) {
    ...
    if (-not (Wait-ForEndpoint -Url 'http://127.0.0.1:11434/api/tags' -Attempts 10 -DelaySeconds 3)) {
        if (-not (Test-Path $ollamaExe)) {
            throw "Ollama did not start from Docker and no local Ollama install was found. Run .\scripts\setup-selfhosted-ai.ps1 once, or install Ollama from https://ollama.com/download/windows."
        }
