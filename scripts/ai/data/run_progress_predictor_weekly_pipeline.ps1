...
powershell -ExecutionPolicy Bypass -File scripts\ai\data\retrain_progress_predictor_real_data.ps1 -JsonlOut $JsonlOut -CsvOut $CsvOut -ModelOut $ModelOut
if ($LASTEXITCODE -ne 0) {
    throw "Retrain script failed with exit code $LASTEXITCODE"
}

Write-Host "==> Step 2/4: Holdout evaluation (group split by user)"
python scripts/ai/training/evaluate_progress_predictor_holdout.py --data $JsonlOut --out-dir tmp --tag $stamp
if ($LASTEXITCODE -ne 0) {
    throw "Holdout evaluation failed with exit code $LASTEXITCODE"
}

Write-Host "==> Step 3/4: Latest-$MonitorLimit monitoring snapshot"
php scripts/ai/data/progress_predictor_monitor_latest.php --limit=$MonitorLimit --threshold=$MonitorThreshold
if ($LASTEXITCODE -ne 0) {
