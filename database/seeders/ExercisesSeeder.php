...
class ExercisesSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // ---- headers
        $header = fgetcsv($fh);
        if (!$header) { fclose($fh); $this->command->warn("ExercisesSeeder: Empty CSV — skipping."); return; }
        ...
        $pushBatch = function () use (&$batch, $now) {
            ...
            foreach ($batch as $r) {
                ...
                if (!isset($dict[$k])) { $dict[$k] = $r; continue; }
        ...
        while (($data = fgetcsv($fh)) !== false) {
            ...
            if (count($batch) >= $batchSize) $pushBatch();
        }
    ...
    private function take(array $row, array $keys): ?string {
        foreach ($keys as $k) if (array_key_exists($k,$row) && $this->hasText($row[$k])) return trim((string)$row[$k]);
        return null;
    }
    private function hasText($v): bool { return is_string($v) ? trim($v) !== '' : ($v !== null && $v !== ''); }
    ...
    private function normalizeEquipment(?string $v): ?string {
        if (!$this->hasText($v)) return 'Bodyweight';
        $s = strtolower(trim($v));
        $map = [
    ...
    private function canonMuscle(?string $m, ?string $exerciseName = null): array {
        if (!$this->hasText($m)) return [null, null];
        $s = strtolower(trim($m));
        $map = [
        ...
        if (isset($map[$s])) return [$map[$s], $s];
        foreach ($map as $k=>$val) if (str_contains($s,$k)) return [$val,$k];

        if (in_array($s, ['arm','arms']) && $this->hasText($exerciseName)) {
            $n = strtolower($exerciseName);
            if ($this->containsAny($n, ['curl'])) return ['biceps', 'heuristic'];
            if ($this->containsAny($n, ['extension','pressdown','skullcrusher','kickback','dip'])) return ['triceps', 'heuristic'];
            return ['biceps', 'heuristic'];
        }
        return [null, null];
    }
    private function containsAny(string $h, array $needles): bool { foreach ($needles as $n) if (str_contains($h,$n)) return true; return false; }

    private function cleanVideoUrl(?string $url): ?string {
        if (!$this->hasText($url)) return null;
        $url = preg_replace('/\s+/', '', trim($url));
        $p = @parse_url($url); if (!$p || empty($p['host'])) return null;
        $h = strtolower($p['host']); $yt = str_contains($h,'youtube.com') || str_contains($h,'youtu.be'); $vm = str_contains($h,'vimeo.com');
        if ($yt) {
            ...
            if (!($hasWatch || $hasId || $short)) return null;
        }
        if ($vm) if (!isset($p['path']) || !preg_match('/\/\d{5,}/',$p['path'])) return null;
        return $url;
    }

    private function explodeTokens(?string $blob): array {
        if (!$this->hasText($blob)) return [];
        $out = [];
    ...
    private function buildTags(?string $tagsRaw, array $conditions, ?string $equipment): array {
        $tags = [];
        if ($this->hasText($tagsRaw)) $tags = array_merge($tags, $this->explodeTokens($tagsRaw));
        $tags = array_merge($tags, $conditions); // include conditions in tags as requested
        if ($this->hasText($equipment)) {
            $eq = strtolower($equipment);
            if (in_array($eq, ['bodyweight','no equipment','none'])) $tags[] = 'bodyweight';
            $tags[] = $this->token($eq);
        }
