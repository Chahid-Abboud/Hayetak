...
class RegisterWizardController extends Controller
{
    ...
    {
        // Validate inputs (mirrors your wizard).
        ...
        $fullName = trim($validated['first_name'].' '.$validated['last_name']) ?: strtok($validated['email'], '@');

        $user = User::create([
            // Keep `name` filled to satisfy NOT NULL
            'name'       => $fullName,
            ...
            'allergies'    => $validated['allergies'] ?? null, // casted to JSON by model
            ...
            'diet_failure_reasons' => $validated['diet_failure_reasons'] ?? null, // casted to JSON
        ...
        // Log in and force 2FA setup
        Auth::login($user);
        $request->session()->regenerate();

        return redirect()
            ->route('two-factor.show')->with('must_enable_2fa', true);
    }
}
