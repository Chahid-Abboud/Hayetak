...
class AiTestGeneratePlans extends Command
{
    ...
    public function handle(PlannerService $planner, PlannerHealthService $health): int
    {
        ...
        if (
            ...
            } elseif ((bool) data_get($healthSnapshot, 'checks.local_fallback.ready', false)) {
                $this->warn('Primary Ollama planner is not reachable; deterministic local fallback is ready.');
            }
        }
