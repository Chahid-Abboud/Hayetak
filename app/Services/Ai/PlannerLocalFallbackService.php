...
class PlannerLocalFallbackService
{
    ...
    public function build(User $user, array $profile, int $planHorizonDays, array $context = []): array
    {
        ...
        $dietDays = $this->buildDietDays($days, $targets, $dietType, $blockedNeedles, $mealCatalog);
        $workout = $this->buildWorkout($profile, $injuries, $exerciseCatalog, $location, $availableEquipment, $goalText);

        return [
            'overview' => [
                'summary' => 'A complete safety-first plan was generated by the local planner engine because the model provider was unavailable.',
            ...
            'safety' => [
                'hard_rules_observed' => [
                    ...
                    'Fallback output remains schema-compliant and practical.',
                ],
            ...
            'ml_readiness' => [
                ...
                'notes' => 'Local planner output is deterministic and supports reliable continuity when the model provider is unavailable.',
            ],
        ];
    }
    ...
    private function buildDietDays(int $days, array $targets, string $dietType, array $blockedNeedles, array $mealCatalog): array
    {
        ...
        for ($day = 1; $day <= $days; $day++) {
            ...
            foreach ($mealCodes as $index => $mealCode) {
                ...
                if ($mealCode === 'snack') {
                    $selected = $this->pickUniqueSnackName($candidates, $day + $index, $lastUsed[$mealCode] ?? null, $usedSnackNames);
                    $usedSnackNames[] = strtolower($selected);
                } else {
                    $selected = $this->pickMealName($candidates, $day + $index, $lastUsed[$mealCode] ?? null);
                }
                ...
                $meals[] = [
                    ...
                    'items' => [[
                        'name' => $selected,
                        'portion' => $mealCode === 'snack' ? '1 portion' : '1 serving',
                        'calories_kcal' => $targetKcal,
                        'protein_g' => max(8, (int) round($targets['protein_g'] * $ratio)),
                        'carbs_g' => max(6, (int) round($targets['carbs_g'] * $ratio)),
                        'fat_g' => max(3, (int) round($targets['fat_g'] * $ratio)),
                        'recipe_note' => $this->recipeNoteForMeal($mealCode),
                        'search_terms' => [$selected, 'healthy '.$mealCode, 'high protein '.$mealCode],
                        'alternatives' => $this->mealAlternatives($candidates, $selected),
                    ]],
                ];
            }
        ...
        return [
            ...
            'snack' => $isVegan
                ? [
                    ...
                    'Tofu cocoa mousse cup',
                    'Edamame and cucumber cup',
                    'Chia pudding with berries',
                    'Oat energy bites',
                    'Trail mix with pumpkin seeds and raisins',
                    ...
                    'Light vegan wafer and fruit cup',
                ]
                : [
                    ...
                    'Cheese and wholegrain toast bites',
                    'Turkey roll-ups with lettuce',
                    'Fruit salad with yogurt',
                    'Dark chocolate square with berries',
                    'Light protein wafer bar and apple',
                ],
        ];
    }
    ...
    private function buildWorkout(array $profile, array $injuries, array $exerciseCatalog, string $location, array $availableEquipment, string $goalText): array
    {
        ...
        for ($dayIndex = 1, $trainCounter = 0; $dayIndex <= 7; $dayIndex++) {
            ...
            $split = $splits[$trainCounter % count($splits)];
            $trainCounter++;
            $exercises = $this->buildTrainingDayExercises($pool, $dayIndex, $location, $split['sequence'], $goalText);
    ...
    private function buildTrainingDayExercises(array $pool, int $dayIndex, string $location, array $sequence, string $goalText): array
    {
        $count = count($pool);
        if ($count === 0) {
            return [];
